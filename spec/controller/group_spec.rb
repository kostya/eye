...
describe "Eye::Group" do
  ...
  describe "Chain calls" do
    ...
    it "config for start and restart, use both" do
      ...
      mock(@g).chain_schedule(:sync, 8, {command: :restart}, nil)
      @g.restart
    end

    it "should use options type" do
      @g = Eye::Group.new('gr', {:chain => {:start => {:type => :sync, :command => :start}}})
      mock(@g).chain_schedule(:sync, Eye::Group::DEFAULT_CHAIN, {command: :start}, nil)
      @g.start
    end

    it "with empty grace, should call default grace 0" do
      @g = Eye::Group.new('gr', {:chain => {:start => {:command => :start}}})
      mock(@g).chain_schedule(:async, Eye::Group::DEFAULT_CHAIN, {command: :start}, nil)
      @g.start
    end
    ...
    it "chain options for restart, but called start, should call chain but with default options" do
      ...
      mock(@g).chain_schedule(:async, Eye::Group::DEFAULT_CHAIN, {command: :restart}, nil)
      @g.restart
    end
    ...
    it "when chain clearing by force" do
      ...
      mock(@g).chain_schedule(:async, 0, {command: :monitor}, nil)
      @g.monitor

      mock(@g).chain_schedule(:async, 0, {command: :restart}, nil)
      @g.restart

      mock(@g).chain_schedule(:async, 0, {command: :start}, nil)
      @g.start
    end
    ...
    describe "monitor using chain as start" do
      it "monitor call chain" do
        ...
        mock(@g).chain_schedule(:async, 3, {command: :monitor}, nil)
        @g.monitor
      end

      it "monitor not call chain" do
        @g = Eye::Group.new('gr', {:chain => {:restart => {:command => :restart, :grace => 3}}})
        mock(@g).chain_schedule(:async, Eye::Group::DEFAULT_CHAIN, {command: :monitor}, nil)
        @g.monitor
      end
    end
