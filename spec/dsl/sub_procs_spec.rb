...
describe "sub procs" do
  ...
  it "include lambda with arg" do
    ...
    E
    Eye::Dsl.load(conf).should == {"bla" => {:name=>"bla", :working_dir=>"1"}}
  end
  ...
  it "include part of code" do
    Eye::Dsl.load(nil, fixture("dsl/include_test.eye")).should == {
  ...
  it "include part of code 2 times" do
    Eye::Dsl.load(nil, fixture("dsl/include_test2.eye")).should == {"test2" => {:name=>"test2", :groups=>{"ha"=>{:name=>"ha", :application=>"test2", :environment=>{"a"=>"b"}, :processes=>{"bla"=>{:name=>"bla", :application=>"test2", :environment=>{"a"=>"b"}, :group=>"ha", :pid_file=>"10"}}}}}}
  end
